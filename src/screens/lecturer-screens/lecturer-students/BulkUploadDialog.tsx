...
const BulkUploadDialog = () => {
  ...
  const [selectedCourse, setSelectedCourse] = useState<ILecturerCourse | null>(
    null
  );

  const { isBulkUploadOpen, setIsBulkUploadOpen } = useAdminResultsStore();
  const [getTemplate] = useLazyGetStudentsResultsTemplateQuery();
  const { data: allDepartmentalCourses } =
    useGetCoursesAssignedToLecturerQuery();
  const [bulkUploadTrigger, { isLoading }] = useBulkCreateResultsMutation();
  ...
  const handleUpload = async () => {
    ...
    formData.append("session", selectedCourse.session);
    ...
    try {
      ...
      toast.error(message || "Failed to upload students");
    }
  };
  ...
  const downloadExamFormat = async (format: string) => {
    ...
    try {
      ...
      link.href = url;
      link.download = `student_results_template.${
        format === "txt" ? "txt" : "xlsx"
      }`;
  ...
  const closeBulkDialog = () => setIsBulkUploadOpen(false);
  console.log(allDepartmentalCourses);
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) => {
                const course = allDepartmentalCourses?.courses.find(
                  (c) => c._id === value
                );
            ...
            >
              ...
              <SelectContent>
                {allDepartmentalCourses?.courses?.map((d) => (
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={!file || isLoading || !selectedCourse}
          >
            {isLoading ? (
